#Loading Json File and converting to csv
import pandas as pd
import numpy as np
import json
...
def jsonToCSV():
    ...
    while end != 1000000:
        songPlaylistArray = []
        path = 'path_to_folder_of_json_files/mpd.slice.' + str(start) + "-" + str(end-1) + '.json'
        ...
        songPlaylist.to_csv('path_to_folder/data_csv/' + str(start) + "-" + str(end-1) + '.csv', index=False)
        start += 1000
        end += 1000
